...
export function flattenDocumentToCanvas(doc: Document, opaque = false): HTMLCanvasElement {
  const canvas = document.createElement("canvas");
  canvas.width = doc.width;
  canvas.height = doc.height;
  const ctx = canvas.getContext("2d");
  if (!ctx) return canvas;
  // A background with zero alpha stays transparent, unless the target format
  // cannot carry alpha, in which case it is flattened onto white.
  if (doc.backgroundColor.a > 0) {
...
export async function flattenToBytes(doc: Document, path: string): Promise<Uint8Array> {
  ...
  const mime = isJpeg ? "image/jpeg" : ext === "webp" ? "image/webp" : "image/png";
  const canvas = flattenDocumentToCanvas(doc, isJpeg);
